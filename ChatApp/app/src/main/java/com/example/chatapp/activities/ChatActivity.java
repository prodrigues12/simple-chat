...
public class ChatActivity extends AppCompatActivity {
    ...
    private void sendMessage() {
        ...
        Log.i("INFO", "### no sendMess ConversionID: "+ conversionId);
    ...
    //Revisado 20/05 05:25
    private void loadReceivedDetaisl() {
        receiverUser = (User) getIntent().getSerializableExtra(Constants.KEY_USER);
        binding.textName.setText(receiverUser.name);
    }

    //Revisado 20/05 05:28
    ...
    private void addConversion(HashMap<String, Object> conversion) {
        ...
        Log.i("INFO", "### ConversionID: "+ conversionId);
    }
    ...
    private void checkConversionRemotely(String senderId, String receiverId) {
        database.collection(Constants.KEY_COLLECTION_CONVERSATIONS)
                ...
                .get()
                .addOnCompleteListener(conversionOnCpmpleteListener);

    }

    private final OnCompleteListener<QuerySnapshot>conversionOnCpmpleteListener = task -> {
        if(task.isSuccessful() && task.getResult() !=null && task.getResult().getDocuments().size()>0){
            DocumentSnapshot documentSnapshot = task.getResult().getDocuments().get(0);
            conversionId = documentSnapshot.getId();
            Log.i("INFO", "### ConversionID inserido: "+ conversionId +"docomentSnap: "+ documentSnapshot.getId());
        }
    };
